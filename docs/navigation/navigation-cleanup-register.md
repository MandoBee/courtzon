...
| 2026-08-07 | — | Consumer 5 (player) resolved **no cleanup items**. `BottomNav.tsx` continues to key React list items by `item.path` (NC-002 pattern — unchanged, low risk). Register unchanged — burndown still 4 open / 0 in progress / 0 completed. |
| 2026-08-07 | NC-003 | **RESOLVED** — Consumer 6 (workspace) replaced `buildSections()` (parallel navigation tree) with `resolveWorkspaceNav(t)` consuming `ADMIN_NAV` (commit `d031f33`). The DnD editor is now Registry-driven with `nav.admin.*` ids. Burndown: **3 open / 0 in progress / 1 completed.** |
