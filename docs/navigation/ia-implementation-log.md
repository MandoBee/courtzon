...
| Commit 2 | People | People domain modules reordered: Users → Roles → Organisations → Membership → CRM → HR | Logical flow: fundamental entities (Users) → access control (Roles) → organizational units (Organisations) → member management (Membership) → customer relationships (CRM) → staff (HR). All module IDs, permission keys, paths, labels, and children unchanged. Internal order only. | Business Constitution §1.2 | None |
