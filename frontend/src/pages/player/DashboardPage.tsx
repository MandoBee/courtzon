...
export default function PlayerDashboardPage() {
  ...
  const { data: enrollments, isLoading: loadingEnrollments } = useQuery({
    ...
    queryFn: () => api.get('/my/academy/enrollments').then((r) => r.data?.data || []),
  });

  const { data: tournaments, isLoading: loadingTournaments } = useQuery({
    queryKey: ['my', 'tournaments'],
    queryFn: () => api.get('/my/tournaments').then((r) => r.data?.data || []),
  });

  const { data: recentActivity } = useQuery({
    queryKey: ['my', 'dashboard'],
    queryFn: () => api.get('/my/dashboard').then((r) => r.data?.activity || r.data?.recentActivity || []).catch(() => []),
  });
    ...
    <div className="space-y-5 md:space-y-6 pb-4">
      ...
      <div className="bg-gradient-to-br from-[var(--color-primary)]/10 to-[var(--color-info)]/5 rounded-[var(--radius-lg)] p-5 md:p-6">
        <h1 className="text-xl md:text-2xl font-bold text-[var(--color-text)]">
          {t('player.welcome_back')}, {user?.fullName || t('player.player')}!
        </h1>
        <p className="text-sm text-[var(--color-text-muted)] mt-1">{t('player.dashboard_subtitle')}</p>
      </div>

      {/* Stat Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label={t('player.wallet_balance')} value={wallet?.balance != null ? `${Number(wallet.balance).toFixed(2)} ${wallet.currency || ''}` : '—'} icon="💰" href="/profile" />
        <StatCard label={t('player.unread_notifications')} value={unreadCount ?? '—'} icon="🔔" href="/notifications" />
        <StatCard label={t('player.upcoming_bookings')} value={Array.isArray(upcomingBookings) ? upcomingBookings.length : '—'} icon="📅" href="/bookings" />
        <Can permission="membership.view">
          <StatCard label={t('player.membership')} value={t('player.view_plans')} icon="⭐" href="/membership" />
        </Can>
      </div>

      {/* Quick Actions */}
      <div>
        <h2 className="text-sm font-semibold text-[var(--color-text)] mb-3">{t('player.quick_actions')}</h2>
        <div className="grid grid-cols-4 gap-2">
          <QuickAction label={t('player.book_court')} icon="🎾" to="/bookings?newBooking=true" />
          <QuickAction label={t('player.find_matches')} icon="🤝" to="/matches" />
          <Can permission="academies.view">
            <QuickAction label={t('player.browse_academies')} icon="🎓" to="/academies" />
          </Can>
          <Can permission="tournaments.view">
            <QuickAction label={t('player.browse_tournaments')} icon="🏆" to="/tournaments" />
          </Can>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-[var(--color-text)]">{t('player.upcoming_bookings')}</h2>
        ...
        {loadingBookings ? <SkeletonRow count={2} /> : (
          <div className="space-y-2">
            {Array.isArray(upcomingBookings) && upcomingBookings.length > 0 ? upcomingBookings.slice(0, 3).map((b: any) => (
              ...
              <p className="text-sm text-[var(--color-text-muted)] py-4 text-center">{t('player.no_upcoming_bookings')}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-[var(--color-text)]">{t('player.upcoming_matches')}</h2>
          ...
          <div className="space-y-2">
            {Array.isArray(matches) && matches.length > 0 ? matches.slice(0, 3).map((m: any) => (
              <Link key={m.id} to={`/matches/${m.id}`} className="block bg-[var(--color-surface)] rounded-[var(--radius-lg)] border border-[var(--color-border)] p-3 hover:border-[var(--color-primary)]/30 transition-colors">
                <p className="text-sm font-medium text-[var(--color-text)]">{m.title || m.name || `${t('player.match')} #${m.id}`}</p>
                <p className="text-xs text-[var(--color-text-muted)]">{m.scheduled_date ? new Date(m.scheduled_date).toLocaleDateString() : ''}</p>
              </Link>
            )) : (
              <p className="text-sm text-[var(--color-text-muted)] py-4 text-center">{t('player.no_upcoming_matches')}</p>
            )}
          </div>
        )}
      </div>
      ...
      <Can permission="academies.enroll">
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-[var(--color-text)]">{t('player.active_enrollments')}</h2>
            ...
            <div className="space-y-2">
              {Array.isArray(enrollments) && enrollments.length > 0 ? enrollments.slice(0, 3).map((e: any) => (
                <div key={e.id} className="bg-[var(--color-surface)] rounded-[var(--radius-lg)] border border-[var(--color-border)] p-3">
                  <p className="text-sm font-medium text-[var(--color-text)]">{e.program_name || e.academy_name || t('player.academy')}</p>
                  <p className="text-xs text-[var(--color-text-muted)]">{e.status}</p>
                </div>
              )) : (
                <p className="text-sm text-[var(--color-text-muted)] py-4 text-center">{t('player.no_enrollments')}</p>
              )}
            </div>
          )}
        </div>
      ...
      <Can permission="tournaments.register">
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-[var(--color-text)]">{t('player.active_tournaments')}</h2>
          ...
          {loadingTournaments ? <SkeletonRow count={1} /> : (
            <div className="space-y-2">
              {Array.isArray(tournaments) && tournaments.length > 0 ? tournaments.slice(0, 3).map((t: any) => (
                ...
                <p className="text-sm text-[var(--color-text-muted)] py-4 text-center">{t('player.no_tournaments')}</p>
              )}
            </div>
          )}
        </div>
      ...
      {/* Recent Activity */}
      <div>
        <h2 className="text-sm font-semibold text-[var(--color-text)] mb-3">{t('player.recent_activity')}</h2>
          ...
          <div className="space-y-2">
            {recentActivity.slice(0, 5).map((a: any, i: number) => (
              <div key={a.id ?? i} className="flex items-center gap-3 bg-[var(--color-surface)] rounded-[var(--radius-lg)] border border-[var(--color-border)] p-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-[var(--color-text)] truncate">{a.description || a.message || a.action}</p>
                  <p className="text-xs text-[var(--color-text-muted)]">{a.created_at ? new Date(a.created_at).toLocaleDateString() : ''}</p>
                </div>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-sm text-[var(--color-text-muted)] py-4 text-center">{t('player.no_recent_activity')}</p>
        )}
      </div>
    </div>
  );
