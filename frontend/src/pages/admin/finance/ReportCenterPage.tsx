...
export default function ReportCenterPage() {
  ...
  const { from, to } = apiDateRange(RANGE_DAYS[range] || 30);
  ...
  const { data: ledger } = useQuery({
    ...
    enabled: tab === 'revenue' || tab === 'wallet',
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-[var(--color-text)]">Report Center</h1>
      ...
      <div className="flex gap-1 bg-[var(--color-bg)] rounded-lg p-1 w-fit">
        {(['revenue', 'wallet', 'settlements'] as ReportTab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-1.5 text-sm rounded-md font-medium capitalize ${tab === t ? 'bg-[var(--color-surface)] shadow text-[var(--color-text)]' : 'text-[var(--color-text-muted)]'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {Object.keys(RANGE_DAYS).map(r => (
          <button key={r} onClick={() => setRange(r)}
            className={`px-3 py-1 text-xs rounded-md font-medium ${range === r ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg)] text-[var(--color-text-muted)]'}`}>
            {r}
          </button>
        ))}
      </div>
      ...
      {tab === 'revenue' && (
        <div className="space-y-6">
          ...
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
            ...
            {entries.length === 0 ? (
              <p className="text-sm text-[var(--color-text-muted)] text-center py-4">No ledger entries.</p>
            ) : (
              <table className="w-full text-sm">
                <thead><tr className="border-b border-[var(--color-border)] text-[var(--color-text-muted)] text-xs">
                  <th className="text-left px-2 py-2">Date</th><th className="text-left px-2 py-2">Source</th><th className="text-left px-2 py-2">Account</th><th className="text-left px-2 py-2">Side</th><th className="text-right px-2 py-2">Amount</th>
                </tr></thead>
            ...
            {walletSummary ? (
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-lg font-bold text-[var(--color-text)]">EGP {Number(walletSummary.totalBalance).toLocaleString()}</p>
                    <p className="text-sm text-[var(--color-text-muted)]">Platform Wallet Liability · {walletSummary.totalWallets} wallets</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div><p className="text-xs text-[var(--color-text-muted)]">Currency</p><p className="text-sm font-medium">{walletSummary.currencyCode || 'EGP'}</p></div>
              ...
              <p className="text-sm text-[var(--color-text-muted)] text-center py-8">Loading wallet summary...</p>
            )}
          </div>
