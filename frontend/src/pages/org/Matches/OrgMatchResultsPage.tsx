...
import { fetchOrgResults, resolveOrgDispute, correctOrgResult, fetchSportFormats } from '../../../services/match-result.api';
...
export default function OrgMatchResultsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['org-match-results', orgId, status],
    queryFn: () => fetchOrgResults(orgId!, status || undefined),
    enabled: !!orgId,
  });
  ...
  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ['org-match-results', orgId, status] });
  };

  const resolveMutation = useMutation({
    mutationFn: ({ approve, displayResult }: { approve: boolean; displayResult?: RawMatchResultPayload }) =>
      resolveOrgDispute(orgId!, modal!.record!.id, { approve, displayResult, note: note || undefined }),
    onSuccess: () => {
  ...
  const correctMutation = useMutation({
    mutationFn: () => correctOrgResult(orgId!, modal!.record!.id, payload),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6" data-testid="org-match-results-page">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[var(--color-text)]">{t('matchResult.orgTitle')}</h1>
        <select
          value={status}
