...
/**
 * Organisation Accounting Records — READ-ONLY view + canonical Manual Journal.
 *
 * Lists the organisation's journal entries using the SAME canonical grouped
 * data as the Super Admin Journal Entries screen (scoped server-side to the
 * organisation). Authorised org users can also post a balanced MANUAL journal
 * entry through the canonical creation path (orgJournalCreateHandler ->
 * createJournalEntryHandler); the organisation is derived from the route, never
 * from the client, so an org user can only post to their own organisation.
 */
export default function OrgAccountingRecordsPage() {
  ...
  // Organisation-scoped Chart of Accounts for the manual journal account picker
  // (only postable, visible accounts the org is allowed to use).
  ...
  const accountList = useMemo(() => {
    const coa = accountsData || {};
    return [...(coa.global || []), ...(coa.org || [])]
      .filter((a: any) => a.is_postable && (a.customization == null || a.customization.is_visible !== false))
      .map((a: any) => ({ id: a.id, code: a.code, name: a.effective_name || a.name }));
  }, [accountsData]);

  const createMutation = useMutation({
    mutationFn: (payload: any) => api.post(`/org/${orgId}/accounting/journal`, payload).then((r: any) => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['org', 'accounting', 'journal-entries'] });
      resetForm();
  ...
  const resetForm = () => {
    setShowForm(false);
    setForm({ entry_date: localToday(), description: '' });
    setLines([{ account_id: '', account_code: '', account_name: '', debit: 0, credit: 0 }]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.description || !form.entry_date) { showToast('Description and entry date are required', 'error'); return; }
    if (!isBalanced) { showToast('Debits must equal credits', 'error'); return; }
    if (lines.some((l) => !l.account_id)) { showToast('All line items must have an account selected', 'error'); return; }
    // The organisation is NOT sent by the client — the backend derives it from
    // the authenticated route context (orgJournalCreateHandler forces :orgId).
    ...
    <Can permission="org.accounting.view">
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          {can('org.accounting.journal.create') && (
            <Button onClick={() => { resetForm(); setShowForm(true); }}>+ New Manual Journal</Button>
          )}
        </div>
        ...
        {showForm && (
          <form onSubmit={handleSubmit}
            ...
            <h3 className="font-semibold text-[var(--color-text)] mb-4">New Manual Journal</h3>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-xs text-[var(--color-text-muted)] mb-1">Entry Date *</label>
                <input type="date" value={form.entry_date} onChange={(e) => setForm({ ...form, entry_date: e.target.value })} required aria-label="Entry Date"
              ...
              <div>
                ...
                <input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required aria-label="Description"
                  className="w-full px-3 py-2 border rounded-[var(--radius-md)] bg-[var(--color-bg)] text-sm" />
              </div>
            </div>
