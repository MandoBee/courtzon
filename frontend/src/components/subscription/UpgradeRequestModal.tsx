...
export default function UpgradeRequestModal({ orgId, open, onClose, triggerMessage }: Props) {
  ...
  const upgradeMutation = useMutation({
    mutationFn: (data: { planId: number; notes?: string }) =>
      api.post(`/org/${orgId}/subscription/request-upgrade`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['org-subscription'] });
      showToast('Upgrade request submitted! An admin will review it.');
      onClose();
    },
    onError: (err: any) => {
  ...
  const handleSubmit = () => {
    ...
    upgradeMutation.mutate({ planId: selectedPlanId, notes: notes || undefined });
  };

  const currentPlan = subscription;
  const pendingUpgrade = currentPlan?.pendingUpgrade;
  const plans = (availablePlans || []).filter((p: any) => p.id !== currentPlan?.planId);
      ...
      {subLoading ? (
        ...
        <div className="space-y-4">
          ...
          {plansLoading ? (
            ...
            <>
              <h3 className="text-sm font-semibold text-[var(--color-text)]">Choose a Plan to Upgrade</h3>
              ...
              <button
                ...
                disabled={!selectedPlanId || upgradeMutation.isPending || !!pendingUpgrade}
