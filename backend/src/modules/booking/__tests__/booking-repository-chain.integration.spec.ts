...
describe('Repository → TimeEngine contract: findBookingsByBusinessDate returns camelCase', () => {
  ...
  it('detects pending_payment bookings as conflicts', async () => {
    const pool = getPool();
    ...
    // Insert a pending_payment booking (blocks availability like confirmed)
    await pool.execute(
      `INSERT INTO bookings (user_id, organisation_id, resource_id, branch_id, booking_type,
        booking_date, business_date, start_time, end_time, start_at_utc, end_at_utc,
        total_amount, payment_status, booking_status, expires_at)
       VALUES (1, 1, ?, ?, 'public_match', ?, ?, '14:00', '15:00',
        CONCAT(?, 'T12:00:00.000Z'), CONCAT(?, 'T13:00:00.000Z'),
        100.00, 'pending', 'pending_payment', DATE_ADD(NOW(), INTERVAL 30 MINUTE))`,
      [resourceId, branchId, dateStr, dateStr, dateStr, dateStr]
    );
    ...
    try {
      ...
      // The 14:00-15:00 local slot (12:00-13:00 UTC) must be booked by the pending_payment booking
      const targetSlot = resolved.find(
        (s) => s.localStartTime === '14:00' && s.localEndTime === '15:00'
      );
      expect(targetSlot).toBeDefined();
      expect(targetSlot!.status).toBe('booked');
    } finally {
      await pool.execute(
        `DELETE FROM bookings WHERE resource_id = ? AND booking_status = 'pending_payment'`,
        [resourceId]
      );
    }
  });

  it('all booking statuses blocked: confirmed, pending_payment', async () => {
    const pool = getPool();
    ...
    // Insert pending_payment booking (10:00-11:00 UTC = 13:00-14:00 local)
    await pool.execute(
      `INSERT INTO bookings (user_id, organisation_id, resource_id, branch_id, booking_type,
        booking_date, business_date, start_time, end_time, start_at_utc, end_at_utc,
        total_amount, payment_status, booking_status, expires_at)
       VALUES (1, 1, ?, ?, 'public_match', ?, ?, '13:00', '14:00',
    ...
    try {
      ...
      // 13:00-14:00 booked (pending_payment)
      ...
      await pool.execute(
        `DELETE FROM bookings WHERE resource_id = ? AND booking_status IN ('confirmed', 'pending_payment') AND user_id = 1`,
        [resourceId]
      );
    }
  });
