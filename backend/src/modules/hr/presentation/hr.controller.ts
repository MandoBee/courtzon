    ...
    if (periods.length) {
      ...
      for (const entry of entries) {
        ...
        await conn.execute(
          ...
          [leDrId, periodId, salaryExpenseId, run.period_end, amount, 0, Number(id), description, userId]
        );
        ...
        await conn.execute(
          `INSERT INTO general_ledger (ledger_entry_id, organisation_id, period_id, account_id, entry_date, debit, credit, balance, reference_type, reference_id, description, created_by)
           VALUES (?, NULL, ?, ?, ?, ?, ?, 0, 'payroll', ?, ?, ?)`,
          [leCrId, periodId, salaryPayableId, run.period_end, 0, amount, Number(id), description, userId]
        );
      }
    }
