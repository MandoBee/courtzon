...
CREATE TABLE `bookings` (
  ...
  `booking_status` enum('pending','pending_payment','confirmed','cancelled','completed','expired','checked_in','no_show') DEFAULT 'pending',
...
CREATE TABLE `organisation_subscriptions` (
  ...
  `subscription_status` enum('active','suspended','pending','expired','cancelled') NOT NULL DEFAULT 'pending',
  `auto_renew` tinyint(1) DEFAULT 1,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `last_reminder_sent` varchar(50) DEFAULT NULL COMMENT 'Comma-separated intervals already notified (30,14,7,5,3,1)',
...
CREATE TABLE `organisation_upgrade_requests` (
  ...
  `request_type` enum('NEW_SUBSCRIPTION','PLAN_CHANGE','RENEWAL') DEFAULT NULL,
  ...
  `requested_billing_cycle` varchar(50) DEFAULT NULL,
  ...
  `current_billing_cycle` varchar(50) DEFAULT NULL,
...
CREATE TABLE `payment_transactions` (
  ...
  `payment_status` enum('created','pending','processing','paid','failed','cancelled','expired','refunded') NOT NULL DEFAULT 'created',
  ...
  KEY `idx_order` (`order_id`),
  KEY `idx_reference` (`reference_type`,`reference_id`),
  KEY `idx_pt_status_created` (`payment_status`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `player_profiles` (
  ...
  `playing_hand` enum('right','left','ambidextrous') DEFAULT NULL,
...
CREATE TABLE `transactions` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `public_id` char(36) DEFAULT NULL,
  `type` enum('booking_payment','wallet_topup','refund','payout','marketplace_order','withdrawal','wallet_payment','subscription') NOT NULL,
  `source_type` enum('booking','academy','marketplace','admin','wallet','order','settlement','organisation_upgrade_request') DEFAULT NULL,
