...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `tournament_draw_entries`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
...
CREATE TABLE `tournaments` (
  ...
  `waitlist_enabled` tinyint(1) NOT NULL DEFAULT '0' COMMENT 'When full and enabled, new registrations enter a FIFO waitlist instead of erroring',
...
CREATE TABLE `sport_rule_sets` (
  ...
  `standings_rules` json DEFAULT NULL COMMENT 'Standings/tie-break criteria (per Sport + Format)',
  `is_active` tinyint(1) NOT NULL DEFAULT '1',
  `is_default` tinyint(1) NOT NULL DEFAULT '0',
  `created_by` int unsigned DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `match_result_records` (
  ...
  `rules_snapshot` json NOT NULL COMMENT 'Historical Rules Snapshot (mandatory)',
  `match_type` enum('public','tournament','league') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'public',
  `played_at` datetime NOT NULL COMMENT 'Actual match occurrence/end time - starts the 3-day submission window',
  `branch_id` int unsigned DEFAULT NULL,
  `resource_id` int unsigned DEFAULT NULL,
  ...
  `raw_result` json NOT NULL COMMENT 'Full raw result details per Sport+Format+Rules Version',
  ...
  `submission_deadline_at` datetime DEFAULT NULL COMMENT 'played_at + 3 days',
  `auto_approval_deadline_at` datetime DEFAULT NULL COMMENT 'submitted_at + 3 days',
...
CREATE TABLE `match_result_participants` (
  ...
  `match_evidence` decimal(5,2) DEFAULT NULL COMMENT 'Win=100, Draw=50, Loss=0',
  `evidence_counted` tinyint(1) NOT NULL DEFAULT '0',
  `rating_snapshot_percent` decimal(5,2) DEFAULT NULL COMMENT 'Historical Rating Snapshot at match time',
...
CREATE TABLE `rating_evidence` (
  ...
  `occurred_at` datetime NOT NULL COMMENT 'Decay anchor - half-life = 3 months',
...
-- ---------------------------------------------------------------------------
-- Match Result System — Default Sport Formats + Rule Sets (Part A).
-- ---------------------------------------------------------------------------
...
-- ============================================================================
-- ACADEMY G1 (Migration 158) — ownership + coach setup + confirmation foundation
